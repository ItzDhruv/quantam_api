...
/// ----------------------------------------------------------------
pub(crate) fn encrypt_bytes_for_recipients(
    plaintext: &[u8],
    ...
    //  Kyber wrap ChaCha key (one wrapped copy per recipient)
    for (idx, pk_bytes) in kyber_public_keys.iter().enumerate() {
        ...
        // Derive the key-encryption key from the Kyber shared secret.
        ...
        kyber_outputs.push(KyberEncryptedKey {
            ...
            wrap_nonce: hex::encode(wrap_nonce_bytes), //  STORE NONCE
        });
    }
...
pub async fn encrypt_file_handler(
    ...
    loop {
        let field = match multipart.next_field().await {
            ...
            Ok(None) => break,
            Err(_) => {
                return (StatusCode::BAD_REQUEST, "malformed multipart request")
                    .into_response();
            }
        };
        ...
        match field.name().unwrap_or("") {
            ...
            "file" => {
                file_bytes = match field.bytes().await {
                    ...
                    Err(_) => {
                        return (StatusCode::BAD_REQUEST, "failed to read file field")
                            .into_response();
                    }
                };
                println!(
                ...
                let value = match field.text().await {
                    ...
                    Err(_) => {
                        return (
                            ...
                            "failed to read kyber_pubkeys field",
                        )
                            .into_response();
                    }
                };
